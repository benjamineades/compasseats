/**
 * CompassEats — Restaurant List Ingestion (Google Apps Script)  v3
 * ================================================================
 ...
 *   Two new OPTIONAL columns: price_symbol_raw, price_source_url.
 *   Standing rule going forward: whenever a source you're ingesting shows a
 *   per-venue price indicator (a symbol tier, a numeric range, a stated
 *   average spend — whatever the source itself displays), capture it here
 *   AS-IS, plus the exact URL you saw it on. Do NOT convert it to
 *   CompassEats' own price_band scale yourself — that's a separate,
 *   deliberate normalization step. These two columns are a raw record /
 *   provenance trail, nothing more. They do NOT currently feed
 *   venues.price_tier — price_band still does that job, unchanged. Think of
 *   this as capturing the receipt now so a future normalization pass has
 *   real source data to work from, instead of needing a separate paid
 *   backfill to reconstruct it later.
 *   Appended at the very END of the column list on purpose — reshape's
 *   parser reads Restaurant Awards by column position, so nothing already
 *   in production shifts.
 *
 ...
 *   paste it with the right source_slug and put its grade in category_override.
 ...
 *   build's validation gate rejects those venues. Both are required — the
 *   "nothing else" framing below only ever applied to new grading formats,
 *   not new sources.
 *
 * TWO FUNCTIONS
 ...
 *   with headers" branch below won't fire for it. Add the two new headers
 *   by hand: cell J1 = price_symbol_raw, cell K1 = price_source_url.
 */
...
function makeRestaurantImportTemplate() {
  ...
  var note =
    ...
    '   e.g. "€40-60", "Moderate" -- do not normalize this yourself, just\n' +
    '   copy what the source said. Provenance only, not read by reshape yet.\n' +
...
function ingestRestaurantLists() {
  ...
  var msg =
    'Restaurant ingestion complete (v3 — writes to "Restaurant Awards").\n' +
