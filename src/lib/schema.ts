...
export const AWARD_SOURCES = [
  ...
  { slug: "top-500-bars", name: "Top 500 Bars", tier: "global" },
...
export const RegionCitySchema = z.object({
  ...
  display: z.string().min(1),             // "Florence"
  country: z.string().min(1),             // "Italy"
  country_code: z.string().length(2).toUpperCase(),  // "IT"
...
/**
 * A geographic region (Tuscany, Napa Valley, etc.) grouping multiple cities.
 * Generated by generateRegionsTab.gs → data/regions.json.
 */
export const RegionSchema = z.object({
  slug: SlugSchema,               // "tuscany"
  display: z.string().min(1),     // "Tuscany"
  country: z.string().optional(), // primary country; undefined for multi-country regions
  center_lat: z.number().min(-90).max(90),   // centroid for map default view
  center_lng: z.number().min(-180).max(180),
  venue_count: z.number().int().nonnegative(), // total across all member cities
  city_count: z.number().int().nonnegative(),
  cities: z.array(RegionCitySchema),           // sorted by venue_count desc
});
